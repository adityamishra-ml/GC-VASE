...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        fine_tune_loader = CustomLoader(data_dict, split='fine_tune')  # Fine-tune loader
        #fine_tune_loader = CustomLoader(data_dict, split='train', exclude_subjects=[1, 2, 3, 6, 8, 9, 10, 11, 12, 13, 16, 17, 19, 21, 24, 25, 28, 31, 32, 34, 35, 36, 37, 38, 39, 40])
        del data_dict
    ...
    model.encoder.freeze_gnn_layers()
    model.decoder.freeze_gnn_layers()

    print("Starting fine-tuning...")
    fine_tune_optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr)
    
    for epoch in range(args.fine_tune_epochs):
        model.train()
        print(f"Epoch {epoch+1}/{args.fine_tune_epochs}")
        dataloader = fine_tune_loader.get_dataloader(batch_size=model.batch_size)
        num_batches = len(dataloader)
        with tqdm(total=num_batches, unit_scale=True, disable=not USE_TQDM) as pbar:
            for i, (idxs, xs, ss, ts, rs) in enumerate(dataloader):
                xs = xs.cuda()
                ss = ss.cuda()
                ts = ts.cuda()
                x_dict = {'x': xs, 'S': ss-1, 'T': ts}
                fine_tune_optimizer.zero_grad()
                x, loss_dict = model.losses(x_dict)
                total_loss = sum((model.loss_weights[v] * loss_dict[v] for v in model.used_losses))
                total_loss.backward()
