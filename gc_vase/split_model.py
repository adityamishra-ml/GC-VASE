...
from torch_geometric.nn import GCNConv, global_mean_pool  # Import GNN components
...
class StridedConvolutionalEncoder(nn.Module):
    ...
    def __init__(self, channels, kernel_size, num_layers):
        ...
        self.gnn_layers = nn.ModuleList([GCNConv(channels, channels) for _ in range(3)])  # Add GNN layers
    
    def forward(self, x, edge_index=None, batch=None, cond=None):
        x = self.conv_layers(x)
        if edge_index is not None and batch is not None:
            x = x.transpose(1, 2)  # GCNConv expects (N, C) format
            for gnn in self.gnn_layers:
                x = gnn(x, edge_index)
                x = torch.relu(x)  # Add activation function if needed
            x = global_mean_pool(x, batch)  # Aggregate features for each graph
            x = x.unsqueeze(2).transpose(1, 2)  # Reshape back to (B, C, L) format
        return x
...
class TransformerBottleneck(nn.Module):
    ...
    def __init__(self, channels, num_layers, num_seq):
        ...
        self.transformer_layers = nn.ModuleList([get_transformer_layer() for _ in range(num_layers)])
