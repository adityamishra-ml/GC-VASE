...
def plot_latents(fig, ax, latents, which_values, which='subject', size=1, color_palette=None, edgecolor='black', linewidth=0.5):
    task_to_label = {
        0: 'ERN/Incorrect',
        1: 'ERN/Correct',
        2: 'LRP/Contralateral',
    ...
    if color_palette is None:
        cmap = 'husl'  # Fixed color map to ensure consistency across plots
        #color_palette = sns.color_palette(cmap, len(which_uniques))
        #color_palette = sns.color_palette('Set2', len(which_uniques))
        color_palette = sns.husl_palette(14, l=.5)

    
    idx_trans = {idx: i for i, idx in enumerate(which_uniques)}
    cluster_colors = [color_palette[idx_trans[x]] for x in which_values]
    ...
    for i in which_uniques:
        ...
        label = task_to_label[i] if which == 'task' else f"S{i}"
        ax.scatter(x, y, color=c[0], s=size, label=label, alpha=0.5, edgecolor=edgecolor, linewidth=linewidth)

    ax.grid(True)  # Enable grid
    # Leave xticks and yticks to show numbers on the axes
